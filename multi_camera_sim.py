"""Multi-camera simulator — chia 1 video thành 4 góc giả lập 4 camera.
...
Cách dùng:
  python multi_camera_sim.py --video dataset/carPark.mp4 --slots parking_slots_video.json
  python multi_camera_sim.py --video dataset/carPark.mp4 --slots parking_slots_video.json --preview
"""
...
# parking_detector nằm trong detect_car_update/
sys.path.insert(0, str(Path(__file__).resolve().parent / "detect_car_update"))
...
def parse_args():
    ...
    root = base if (base / "dataset").exists() else base.parent

    parser = argparse.ArgumentParser(description="Multi-camera parking simulator")
    parser.add_argument("--video", "-v", default=str(root / "dataset" / "carPark.mp4"))
    parser.add_argument("--slots", "-s", default=str(root / "parking_slots_video.json"),
