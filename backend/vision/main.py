"""TechGAR main demo ΓÇö tracking, Global ID and parking on four virtual cameras.

Chia video gß╗æc th├ánh 4 phß║ºn (top-left, top-right, bottom-left, bottom-right),
tß╗▒ ─æß╗Öng tß║ío file parking_slots cho mß╗ùi camera (tß╗ìa ─æß╗Ö ─æ├ú shift),
v├á vß║╜ dß║Ñu cß╗Öng ß╗ƒ giß╗»a ─æß╗â minh hß╗ìa v├╣ng cß║»t.
...
class CameraSimulator:
    """Giß║ú lß║¡p nhiß╗üu camera bß║▒ng c├ích cß║»t 1 frame th├ánh c├íc v├╣ng.

    Mß╗ùi camera c├│:
      - crop region (x1, y1, x2, y2) tr├¬n frame gß╗æc
      - danh s├ích slots thuß╗Öc vß╗ü camera ─æ├│ (tß╗ìa ─æß╗Ö ─æ├ú shift vß╗ü local)
      - output JSON ri├¬ng
    """
    ...
    def __init__(
        ...
        if not cap.isOpened():
            raise RuntimeError(f"Kh├┤ng mß╗ƒ ─æ╞░ß╗úc video: {source_path}")
        ...
        # T├¡nh ─æiß╗âm cß║»t (giß╗»a frame)
        self.mid_x = self.frame_w // 2
        self.mid_y = self.frame_h // 2

        # V├╣ng crop cho mß╗ùi camera. Overlap is optional for diagnostics only.
        ...
        # Load v├á ph├ón chia slots
        self._load_and_split_slots(slots_file)

    def _load_and_split_slots(self, slots_file: str) -> None:
        """Load slots, ph├ón loß║íi v├áo camera dß╗▒a tr├¬n center, shift tß╗ìa ─æß╗Ö."""
        ...
        if not path.exists():
            ...
            if alt.exists():
                path = alt
        if not path.exists():
            raise FileNotFoundError(f"Kh├┤ng t├¼m thß║Ñy slots file: {slots_file}")
        ...
        # Scale factor (slots file c├│ thß╗â d├╣ng k├¡ch th╞░ß╗¢c kh├íc video)
        sx = self.frame_w / ref_w
        sy = self.frame_h / ref_h

        for cam_id, cam in self.cameras.items():
            cam["slots"] = []
            cam["slots_raw"] = []  # Giß╗» bß║ún gß╗æc ─æß╗â debug

        for slot in all_slots:
            # Scale center vß╗ü video coords
            center_x = slot["center"]["x"] * sx
            center_y = slot["center"]["y"] * sy

            # X├íc ─æß╗ïnh camera n├áo chß╗⌐a center
            cam_id = self._classify_point(center_x, center_y)

            # Scale v├á shift polygon vß╗ü tß╗ìa ─æß╗Ö local camera
        ...
        # Tß║ío file JSON cho mß╗ùi camera
        for cam_id, cam in self.cameras.items():
            ...
            print(f"  ≡ƒô╖ {cam_id} ({cam['name']}): {len(cam['slots'])} slots ΓåÆ {cam_slots_file.name}")

    def _classify_point(self, x: float, y: float) -> str:
        """Ph├ón loß║íi point v├áo camera n├áo dß╗▒a tr├¬n quadrant."""
    ...
    def draw_split_overlay(self, frame: np.ndarray) -> np.ndarray:
        """Vß║╜ dß║Ñu cß╗Öng ß╗ƒ giß╗»a frame ─æß╗â minh hß╗ìa v├╣ng cß║»t 4 camera."""
        out = frame.copy()
        # ─É╞░ß╗¥ng kß║╗ dß╗ìc giß╗»a
        cv2.line(out, (self.mid_x, 0), (self.mid_x, self.frame_h), (0, 255, 255), 2)
        # ─É╞░ß╗¥ng kß║╗ ngang giß╗»a
    ...
    def draw_camera_with_slots(self, cam_frame: np.ndarray, cam_id: str) -> np.ndarray:
        """Vß║╜ polygon ├┤ ─æß╗ù l├¬n frame camera."""
        out = cam_frame.copy()
        cam = self.cameras[cam_id]
...
# ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
#  Preview mode ΓÇö xem tr╞░ß╗¢c 4 camera tr├¬n 1 cß╗¡a sß╗ò
# ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ

def run_preview(sim: CameraSimulator, source: str, loop: bool = True, playback_fps: float = 0.0):
    """Chß║íy preview 4 camera song song tr├¬n 1 cß╗¡a sß╗ò."""
    cap = cv2.VideoCapture(source)
    if not cap.isOpened():
        raise RuntimeError(f"Kh├┤ng mß╗ƒ ─æ╞░ß╗úc: {source}")

    print("\n≡ƒÄ¼ Preview mode ΓÇö nhß║Ñn Q ─æß╗â tho├ít")
    ...
    while True:
        ...
        # Resize cß║ú 4 vß╗ü c├╣ng k├¡ch th╞░ß╗¢c
...
# ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
#  Production mode ΓÇö parking detection + vehicle tracking tr├¬n mß╗ùi camera
# ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
...
def run_detection(sim: CameraSimulator, source: str, args):
    """Chß║íy ensemble parking detection + motion tracking tr├¬n 4 camera."""
    ...
    # ΓöÇΓöÇ Khß╗ƒi tß║ío detector + tracker + binder cho mß╗ùi camera ΓöÇΓöÇ
    ...
    for cam_id, cam in sim.cameras.items():
        if not cam["slots"]:
            print(f"  ΓÜá∩╕Å {cam_id} kh├┤ng c├│ slot n├áo, bß╗Å qua")
            continue
        ...
        # Motion vehicle tracker (mß╗ùi camera c├│ tracker ri├¬ng)
        ...
        print(f"  Γ£à {cam_id}: {len(cam['slots'])} slots | detector + tracker + binder")
    ...
    print(f"\n≡ƒÜÇ Chß║íy tr├¬n {len(detectors)} camera")
    print(f"   Tracking: mß╗ùi frame | Parking detection: mß╗ùi {parking_interval:.2f}s")
    print(f"   JSON output: mß╗ùi {json_interval:.2f}s")
    print("   Nhß║Ñn Q ─æß╗â tho├ít\n")
    ...
    try:
        while True:
            ...
            # ΓöÇΓöÇ 1. Vehicle tracking (mß╗ùi frame, mß╗ùi camera) ΓöÇΓöÇ
            expired_per_cam = {}
            for cam_id, tracker in trackers.items():
                ...
                # B├ío cho manager biß║┐t track n├áo vß╗½a expire (─æß╗â tß║ío handoff)
            ...
            # Cß║¡p nhß║¡t to├án bß╗Ö camera c├╣ng l├║c. Quan trß╗ìng: manager nh├¼n thß║Ñy
            # xe ß╗ƒ camera nguß╗ôn v├á camera ─æ├¡ch trong c├╣ng frame ─æß╗â handoff ngay.
            ...
            # ΓöÇΓöÇ 2. Parking detection (tß║ºn suß║Ñt thß║Ñp h╞ín) ΓöÇΓöÇ
            ...
            # ΓöÇΓöÇ 3. Ghi JSON output ΓöÇΓöÇ
...
def parse_args():
    ...
    parser.add_argument("--slots", "-s", default=str(root / "config" / "parking_slots.json"),
                        help="File JSON chß╗⌐a 69 ├┤ ─æß╗ù (tß╗ìa ─æß╗Ö video gß╗æc)")
    parser.add_argument("--overlap", type=int, default=0,
                        help="Pixel overlap giß╗»a 2 camera kß╗ü nhau (mß║╖c ─æß╗ïnh 0 ─æß╗â tr├ính ID tr├╣ng)")
    parser.add_argument("--output-dir", default=str(root / "runtime_output"),
                        help="Th╞░ mß╗Ñc chß╗⌐a output JSON")

    parser.add_argument("--preview", action="store_true",
                        help="Chß╗ë xem tr╞░ß╗¢c 4 camera (kh├┤ng chß║íy detection)")
    ...
    # Ensemble tuning (gi├í trß╗ï mß║╖c ─æß╗ïnh tß╗½ ensemble_test.py trackbar)
    ...
    parser.add_argument("--ratio-thr", type=float, default=0.20, help="Ng╞░ß╗íng pixel ratio trß╗æng/─æß║ºy")
    parser.add_argument("--edge-thr", type=float, default=0.25, help="Ng╞░ß╗íng edge ratio pass 2")

    # Vehicle tracking
    parser.add_argument("--json-fps", type=float, default=5.0, help="Tß║ºn suß║Ñt ghi JSON output")
    parser.add_argument("--min-visible-count", type=int, default=4, help="Detection li├¬n tiß║┐p ─æß╗â x├íc nhß║¡n xe")
    parser.add_argument("--lost-track-ttl", type=int, default=90, help="Sß╗æ frame giß╗» track lost")
    parser.add_argument("--handoff-ttl", type=int, default=45, help="Sß╗æ frame chß╗¥ xe xuß║Ñt hiß╗çn ß╗ƒ camera kß╗ü")
    parser.add_argument("--handoff-match-distance", type=float, default=100.0, help="Khoß║úng c├ích tß╗æi ─æa khi gh├⌐p handoff (pixel to├án cß╗Ñc)")
    parser.add_argument("--handoff-appearance-threshold", type=float, default=0.45, help="Ng╞░ß╗íng kh├íc biß╗çt HSV tß╗æi ─æa khi gh├⌐p handoff")
    parser.add_argument("--handoff-lookahead-frames", type=int, default=16, help="Sß╗æ frame dß╗▒ ─æo├ín tr╞░ß╗¢c khi xe chß║ím ranh camera")
    parser.add_argument("--handoff-prediction-radius", type=float, default=90.0, help="Sai sß╗æ tß╗æi ─æa cß╗ºa vß╗ï tr├¡ handoff dß╗▒ ─æo├ín (pixel)")
    parser.add_argument("--handoff-min-direction-cosine", type=float, default=0.25, help="Cosine h╞░ß╗¢ng tß╗æi thiß╗âu giß╗»a xe nguß╗ôn/─æ├¡ch khi handoff")
    parser.add_argument("--motion-min-area", type=int, default=900, help="Diß╗çn t├¡ch foreground tß╗æi thiß╗âu")
    parser.add_argument("--motion-max-distance", type=float, default=180.0, help="Gate Kalman (pixel)")
    parser.add_argument("--motion-min-displacement", type=float, default=12.0, help="Pixel tß╗æi thiß╗âu ─æß╗â confirm")
    parser.add_argument("--slot-release-grace", type=int, default=90, help="Sß╗æ frame giß╗» global vehicle_id sau khi ├┤ trß╗æng")
    parser.add_argument("--slot-bind-confirmations", type=int, default=2, help="Sß╗æ lß║ºn detector phß║úi x├íc nhß║¡n tr╞░ß╗¢c khi g├ín xe v├áo ├┤")
    parser.add_argument("--slot-stop-seconds", type=float, default=1.0, help="Sß╗æ gi├óy xe phß║úi ─æß╗⌐ng ß╗òn ─æß╗ïnh tr╞░ß╗¢c khi g├ín ├┤")
    parser.add_argument("--slot-exit-seconds", type=float, default=0.5, help="Sß╗æ gi├óy c├╣ng Global ID ß╗ƒ ngo├ái ROI tr╞░ß╗¢c khi gß╗í override")
    ...
    parser.add_argument("--slot-strong-vehicle-overlap", type=float, default=0.60, help="Overlap ─æß╗º mß║ính khi t├óm bbox nß║▒m ngo├ái ROI")
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("  ≡ƒà┐∩╕Å  Multi-Camera Parking Simulator")
    print("=" * 60)
    ...
    print(f"\n≡ƒôÉ Video: {sim.frame_w}x{sim.frame_h} | Cß║»t tß║íi ({sim.mid_x}, {sim.mid_y})")
    for cam_id, cam in sim.cameras.items():
        crop = cam["crop"]
        print(f"  {cam_id} ({cam['name']}): crop=({crop[0]},{crop[1]})-({crop[2]},{crop[3]}) "
              f"ΓåÆ {cam['width']}x{cam['height']} | {len(cam['slots'])} slots")
