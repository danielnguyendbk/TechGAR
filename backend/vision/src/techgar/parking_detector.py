"""Parking slot detector ΓÇö t├ích logic ensemble detection tß╗½ ensemble_test.py.

Module n├áy cung cß║Ñp class ParkingDetector d├╣ng ─æ╞░ß╗úc nh╞░ library,
kh├┤ng c├│ UI/trackbar/main loop.

Thuß║¡t to├ín:
  1. Ensemble 25 biß║┐n thß╗â (5 delta-gamma ├ù 5 delta-CLAHE) vote trß╗æng/─æß║ºy
  2. Pass 1: Center Cluster Rescue ΓÇö cß╗⌐u xe trß║»ng bß╗ï miss ß╗ƒ center
  3. Pass 2: Edge Recheck ΓÇö cß╗⌐u xe c├│ edge ratio cao
  4. Temporal Smoothing ΓÇö ß╗òn ─æß╗ïnh kß║┐t quß║ú qua nhiß╗üu frame
"""
...
@dataclass
class SlotResult:
    """Kß║┐t quß║ú nhß║¡n diß╗çn cho 1 ├┤ ─æß╗ù."""
    slot_id: str
    occupied: bool
    polygon: np.ndarray          # (N, 2) int32 ΓÇö tß╗ìa ─æß╗Ö ─æ├ú scale
    center: Tuple[int, int]
    vehicle_id: Optional[int] = None   # Sß║╜ ─æ╞░ß╗úc binder g├ín sau


@dataclass
class _PrecomputedROI:
    slot_id: str
    polygon_pts: np.ndarray       # (N, 2) int32 ─æ├ú scale
...
class TemporalSmoother:
    """Smoothing trß║íng th├íi ├┤ ─æß╗ù qua nhiß╗üu frame li├¬n tiß║┐p."""
...
class ParkingDetector:
    """Ensemble parking detection ΓÇö d├╣ng ─æ╞░ß╗úc nh╞░ library.

    Params
    ------
    slots_file : str
        ─É╞░ß╗¥ng dß║½n tß╗¢i file JSON chß╗⌐a danh s├ích ├┤ ─æß╗ù (polygon).
    base_gamma, base_clahe, clahe_grid, ratio_thr, edge_thr : float
        Tham sß╗æ ensemble mß║╖c ─æß╗ïnh.  C├│ thß╗â override mß╗ùi frame.
    smoothing_frames : int
        Sß╗æ frame li├¬n tiß║┐p cß║ºn ─æß╗ông thuß║¡n tr╞░ß╗¢c khi ─æß╗òi trß║íng th├íi.
        0 = tß║»t smoothing.
    """
    ...
    def detect(
        ...
        """Ph├ón t├¡ch 1 frame, trß║ú vß╗ü danh s├ích SlotResult."""
        ...
        # ΓöÇΓöÇ Pass 1: Threshold + Center Cluster ΓöÇΓöÇ
        ...
        # ΓöÇΓöÇ Pass 2: Edge Recheck ΓöÇΓöÇ
    ...
    def get_roi_polygon(self, slot_id: str) -> Optional[np.ndarray]:
        """Trß║ú polygon ─æ├ú scale cho 1 slot ID."""
