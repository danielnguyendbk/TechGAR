"""Fallback tracker cho camera b├úi xe nh├¼n tß╗½ tr├¬n cao.

COCO YOLO kh├┤ng nhß║¡n ─æ╞░ß╗úc xe qu├í nhß╗Å/nh├¼n top-down trong video carPark mß║½u.
Backend n├áy d├╣ng foreground motion ─æß╗â tß║ío detection, nh╞░ng thay Particle Filter
c┼⌐ bß║▒ng Kalman constant-velocity + global assignment (LAPJV) + HSV appearance
histogram ─æß╗â hß║ín chß║┐ ─æß╗òi ID v├á hß╗ù trß╗ú Re-ID ngß║»n hß║ín.
"""
...
class MotionVehicleTracker:
    """Tracker kh├┤ng phß╗Ñ thuß╗Öc model, tß╗æi ╞░u cho camera cß╗æ ─æß╗ïnh c├│ xe chuyß╗ân ─æß╗Öng."""
    ...
    def __init__(
        ...
        self.slot_binder = slot_binder  # Tham chiß║┐u tß╗¢i SlotVehicleBinder
    ...
    def _temporal_motion_mask(self, frame: np.ndarray) -> np.ndarray:
        """Chß╗ë giß╗» pixel thß╗▒c sß╗▒ thay ─æß╗òi giß╗»a hai thß╗¥i ─æiß╗âm.

        MOG2 c├│ thß╗â ─æ├ính dß║Ñu xe ─æß╗ù khi ├ính s├íng/n├⌐n video thay ─æß╗òi. Frame
        difference qua nhiß╗üu frame gi├║p bß╗Å c├íc blob ─æß╗⌐ng y├¬n. Median brightness
        shift ─æ╞░ß╗úc trß╗½ tr╞░ß╗¢c ─æß╗â kh├┤ng coi thay ─æß╗òi ph╞íi s├íng to├án khung l├á xe.
        """
    ...
    def _detect(self, frame: np.ndarray) -> Tuple[List[dict], np.ndarray]:
        ...
        # Motion mask l├á cß╗òng bß║»t buß╗Öc: foreground ─æß╗⌐ng y├¬n kh├┤ng ─æ╞░ß╗úc th├ánh xe.
    ...
    def _create_or_reid(self, detection: dict) -> None:
        ...
        # ΓöÇΓöÇ B╞░ß╗¢c 0: Kiß╗âm tra Slot Binder (╞░u ti├¬n cao nhß║Ñt) ΓöÇΓöÇ
        if self.slot_binder is not None:
            recovered_id = self.slot_binder.try_recover_id(point)
            if recovered_id is not None:
                # Kh├┤i phß╗Ñc track vß╗¢i ID c┼⌐ tß╗½ ├┤ ─æß╗ù
                box = detection["box"]
                ...
                # X├│a khß╗Åi exited nß║┐u c├│
                self._exited_tracks.pop(recovered_id, None)
                return

        # ΓöÇΓöÇ B╞░ß╗¢c 1: Re-ID xe ─æ├ú rß╗¥i khung (appearance) ΓöÇΓöÇ
        candidate = None
        best_distance = 0.18
        ...
        # ΓöÇΓöÇ B╞░ß╗¢c 2: Tß║ío track ho├án to├án mß╗¢i ΓöÇΓöÇ
    ...
    def process_frame(self, frame: np.ndarray):
        ...
        expired_tracks = []  # Danh s├ích tracks vß╗½a expire frame n├áy
        for track_id in expired:
            track = self._tracks.pop(track_id)
            # Blob nhiß╗àu ch╞░a tß╗½ng ─æß╗º ─æiß╗üu kiß╗çn x├íc nhß║¡n kh├┤ng ─æ╞░ß╗úc ─æ╞░a v├áo output.
