"""Vehicle tracking cho b├úi xe bß║▒ng YOLO + BoT-SORT/Re-ID.

Module n├áy thay thß║┐ detector MOG2/contour c┼⌐.  MOG2 chß╗ë nh├¼n thay ─æß╗òi pixel n├¬n
kh├┤ng thß╗â theo xe ─æang ─æß╗⌐ng y├¬n, dß╗à vß╗í khi thay ─æß╗òi ├ính s├íng v├á kh├┤ng c├│ dß╗» liß╗çu
─æß╗â nhß║¡n lß║íi ID sau che khuß║Ñt.  YOLO ph├ít hiß╗çn xe trß╗▒c tiß║┐p; BoT-SORT xß╗¡ l├╜ dß╗▒
─æo├ín chuyß╗ân ─æß╗Öng, IoU, low-confidence association v├á Re-ID theo ngoß║íi h├¼nh.
"""
...
class TrackedVehicle:
    """Mß╗Öt xe vß╗¢i ID do BoT-SORT duy tr├¼.

    ``cx, cy`` l├á ─æiß╗âm tiß║┐p x├║c mß║╖t ─æ╞░ß╗¥ng (giß╗»a cß║ính ─æ├íy bbox), ß╗òn ─æß╗ïnh h╞ín
    t├óm bbox khi camera nh├¼n xi├¬n.  Muß╗æn c├│ toß║í ─æß╗Ö m├⌐t/s╞í ─æß╗ô b├úi, truyß╗ün
    homography v├áo :class:`VehicleTracker`.
    """
...
class VehicleTracker:
    """YOLO detector v├á tracker BoT-SORT stateful cho tß╗½ng frame.

    Khi ``tracker_config`` l├á ``botsort_parking_reid.yaml``, tracker d├╣ng Re-ID
    ─æß╗â cß╗æ giß╗» c├╣ng ID khi xe bß╗ï che ngß║»n hß║ín.  Vß╗¢i camera cß╗æ ─æß╗ïnh, cß║Ñu h├¼nh tß║»t
    camera-motion compensation ─æß╗â tiß║┐t kiß╗çm chi ph├¡.
    """
    ...
    def __init__(
        ...
        # Kh├┤ng ghi settings v├áo AppData cß╗ºa hß╗ç thß╗æng; giß╗» mß╗ìi state trong project.
        ...
        try:
            ...
            raise RuntimeError(
                "Thiß║┐u ultralytics. H├úy c├ái dependencies bß║▒ng: "
        ...
        if not Path(self.model_path).exists():
            raise FileNotFoundError(f"Kh├┤ng t├¼m thß║Ñy YOLO model: {self.model_path}")
        if not Path(self.tracker_config).exists():
            raise FileNotFoundError(f"Kh├┤ng t├¼m thß║Ñy tracker config: {self.tracker_config}")
    ...
    def process_frame(self, frame: np.ndarray) -> Tuple[Dict[int, TrackedVehicle], np.ndarray, List[Tuple[int, TrackedVehicle]]]:
        """Ph├ít hiß╗çn + track mß╗Öt frame. Trß║ú bbox mask ─æß╗â t╞░╞íng th├¡ch UI c┼⌐."""
        self._frame_idx += 1
        kwargs = {
            "persist": True,
